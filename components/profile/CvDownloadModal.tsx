...
// profile rail. Shows a thumbnail of each template; clicking Download
// grabs that one — honouring any colour theme the user already picked on
// the /templates page (stored per-template in localStorage). For full
// control (themes + live preview) it links through to /templates.
export function CvDownloadModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    for (const t of CV_TEMPLATES) {
      try {
        const v = localStorage.getItem(`sahan-theme:cv:${t.id}`);
        if (v) next[t.id] = v;
      } catch { /* private mode */ }
    }
    setThemes(next);
  ...
  function hrefFor(id: string) {
    ...
    return `/api/cv/${id}${theme ? `?theme=${encodeURIComponent(theme)}` : ""}`;
  }
