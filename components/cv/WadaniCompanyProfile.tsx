...
export function WadaniCompanyProfile({ data }: { data: CompanyData }) {
  return (
    <>
    ...
    <section className="page cover">
      ...
      <svg className="cover-topo" width="794" height="1123" aria-hidden>
        {Array.from({ length: 14 }).map((_, i) => (
          <path key={i} d={`M -50 ${200 + i * 60} Q 200 ${140 + i * 60}, 400 ${220 + i * 60} T 850 ${190 + i * 60}`}
            stroke={COLORS.cream} strokeWidth="0.7" fill="none" />
        ))}
      </svg>
      ...
      <div className="cover-seal">
        <svg width="16" height="16" viewBox="0 0 11 11" aria-hidden>
          <circle cx="5.5" cy="5.5" r="5.5" fill={COLORS.sand2} />
          <path d="M3 5.5 L4.7 7.2 L8 4" stroke={COLORS.teal} strokeWidth="1.4" fill="none" strokeLinecap="round" />
        </svg>
...
/* ── Cover ─────────────────────────────────────────────────────── */
.cover {
  background: linear-gradient(165deg, ${COLORS.tealDark} 0%, ${COLORS.teal} 55%, #0c1f38 100%);
  color: ${COLORS.paperOnTeal};
...
.cover-head {
  ...
  color: ${COLORS.sand2}; font-weight: 600;
}
.cover-head-l { display: flex; align-items: center; gap: 3mm; }
.cover-rule { width: 8mm; height: 1px; background: ${COLORS.sand2}; display: inline-block; }
.cover-mono {
...
.cover-seal {
  ...
  border: 1.5px solid ${COLORS.sand}; color: ${COLORS.sand2};
...
.cover-est {
  font-family: "Source Serif 4", "Source Serif Pro", Georgia, serif;
  font-size: 11pt; font-style: italic; color: ${COLORS.sand2};
  letter-spacing: 0.02em; margin-bottom: 6mm;
}
.cover-h1 {
  font-family: "Source Serif 4", "Source Serif Pro", Georgia, serif;
  font-size: 72pt; font-weight: 300; letter-spacing: -0.04em;
  line-height: 0.92; margin: 0; color: ${COLORS.paperOnTeal};
}
.cover-h1-italic { font-style: italic; color: ${COLORS.sand2}; font-weight: 250; }
.cover-tagline {
  margin-top: 8mm; display: flex; align-items: center; gap: 5mm;
  font-weight: 500; font-size: 11pt;
  letter-spacing: 0.04em; color: ${COLORS.paperOnTeal}; text-transform: uppercase;
}
.cover-tagline-rule { width: 15mm; height: 2px; background: ${COLORS.sand}; display: inline-block; }
.cover-foot {
  position: absolute; bottom: 16mm; left: 18mm; right: 18mm;
  padding-top: 5mm; border-top: 1px solid ${COLORS.paperOnTeal}33;
  display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 5mm;
}
.cover-foot-k {
  font-size: 7pt; color: ${COLORS.sand2}; letter-spacing: 0.18em;
  text-transform: uppercase; font-weight: 600;
}
.cover-foot-v {
  font-family: "Source Serif 4", "Source Serif Pro", Georgia, serif;
  font-size: 11pt; font-weight: 400; color: ${COLORS.paperOnTeal};
...
.eyebrow {
  font-size: 8.5pt; color: ${COLORS.sienna};
  letter-spacing: 0.22em; text-transform: uppercase; font-weight: 600;
}
.eyebrow-mb { margin-bottom: 4mm; }
.eyebrow-tiny { font-size: 7pt; letter-spacing: 0.1em; color: ${COLORS.muted}; }
.eyebrow-onDark { color: ${COLORS.sand}; }
.display {
  font-family: "Source Serif 4", "Source Serif Pro", Georgia, serif;
  font-size: 36pt; font-weight: 350; letter-spacing: -0.03em;
  line-height: 0.95; margin: 4mm 0 0; max-width: 140mm;
}
.italic-sienna { font-style: italic; color: ${COLORS.sienna}; }
.about-intro { margin-top: 10mm; }
.about-body {
  font-size: 11pt; line-height: 1.7; color: ${COLORS.inkSoft};
...
.mv-light { background: #fff; border: 1px solid ${COLORS.rule}; }
.mv-dark { background: ${COLORS.teal}; color: #e6ecf3; }
.mv-quote {
  font-family: "Source Serif 4", "Source Serif Pro", Georgia, serif;
...
.list-row {
  ...
  padding: 2mm 0; border-bottom: 1px solid ${COLORS.rule};
}
.list-row-last { border-bottom: none; }
.list-num {
  font-family: "IBM Plex Mono", "Source Code Pro", monospace;
  font-size: 7.5pt; color: ${COLORS.muted}; width: 7mm;
}
.list-item {
  font-family: "Source Serif 4", "Source Serif Pro", Georgia, serif;
...
.client-chip {
  font-family: "Source Serif 4", "Source Serif Pro", Georgia, serif;
  font-size: 11pt; font-weight: 450; color: ${COLORS.ink};
}
.client-chip:not(:last-child)::after { content: " ·"; color: ${COLORS.muted}; }

/* ── Projects page ─────────────────────────────────────────────── */
.proj-intro { margin-top: 10mm; }
.proj-blurb {
  font-size: 10.5pt; color: ${COLORS.muted}; max-width: 140mm;
  margin-top: 4mm; line-height: 1.55;
}
.proj-list { margin-top: 8mm; display: flex; flex-direction: column; gap: 4mm; }
.proj-card {
  background: #fff; border: 1px solid ${COLORS.rule}; border-radius: 8px;
  padding: 5mm 6mm;
  display: grid; grid-template-columns: 16mm 1fr 28mm; gap: 5mm; align-items: flex-start;
  break-inside: avoid; page-break-inside: avoid;
}
.proj-tag {
  width: 16mm; height: 16mm; border-radius: 4px; background: ${COLORS.cream};
  border: 1px solid ${COLORS.rule};
  display: flex; flex-direction: column; align-items: center; justify-content: center;
}
.proj-tag-n {
  font-family: "Source Serif 4", "Source Serif Pro", Georgia, serif;
  font-size: 17pt; font-weight: 400; color: ${COLORS.ink};
  line-height: 1; letter-spacing: -0.02em;
}
.proj-tag-s {
  font-size: 6pt; color: ${COLORS.muted}; letter-spacing: 0.1em;
...
.proj-meta {
  font-size: 9.5pt; color: ${COLORS.sienna}; margin-top: 1mm; font-style: italic;
  font-family: "Source Serif 4", "Source Serif Pro", Georgia, serif;
}
.proj-scope { font-size: 9.5pt; line-height: 1.55; color: ${COLORS.inkSoft}; margin: 2mm 0 0; }
...
.team-row { padding: 2mm 0; border-bottom: 1px solid ${COLORS.rule}; }
.team-name { font-family: "Source Serif 4", Georgia, serif; font-size: 11pt; font-weight: 500; }
.team-role { font-size: 9pt; color: ${COLORS.muted}; margin-top: 0.5mm; }

.certs { margin-top: 8mm; }
.cert-row { font-size: 10pt; color: ${COLORS.inkSoft}; margin-bottom: 2mm; display: flex; align-items: baseline; gap: 2mm; flex-wrap: wrap; }
...
.proj-foot {
  ...
  display: flex; justify-content: space-between;
  font-size: 8pt; color: ${COLORS.muted}; letter-spacing: 0.12em;
  text-transform: uppercase; padding-top: 3mm; border-top: 1px solid ${COLORS.rule};
}
.proj-foot-stamp { color: ${COLORS.sienna}; }

.vbadge, .vbadge-sm {
  display: inline-flex; align-items: center; gap: 1mm;
  font-weight: 500; color: ${COLORS.verifiedFg};
